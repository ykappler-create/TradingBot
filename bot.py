...
try:
    ...
    def log_trade_open(**kw): return _log_trade_open(**kw)
    def log_trade_close(**kw): return _log_trade_close(**kw)
    def log_equity(v: float): return _log_equity(v)
    def log_risk(**kw): return _log_risk(**kw)
    def log_position_snapshot(exposures, avg_leverage): return _log_position_snapshot(exposures, avg_leverage)
except Exception:
    # Fallback-kompatibel zur Bridge
    def log_trade_open(symbol, side, qty, price, leverage, exchange, strategy_id, rationale):
        return _write_event(EVENTS_DIR / "trades", {
            "ts": _now_ms(), "type": "trades", "event": "open",
            "symbol": symbol, "side": side, "qty": qty, "price": price,
            "leverage": leverage, "exchange": exchange,
            "strategy_id": strategy_id, "rationale": rationale,
        })
    def log_trade_close(order_ref, symbol, exit_price, profit, pnl_pct, fees=0.0):
        return _write_event(EVENTS_DIR / "trades", {
            "ts": _now_ms(), "type": "trades", "event": "close",
            "order_ref": order_ref, "symbol": symbol,
            "exit_price": exit_price, "profit": profit, "pnl_pct": pnl_pct, "fees": fees,
        })
    def log_equity(equity: float):
        return _write_event(EVENTS_DIR / "equity", {"ts": _now_ms(), "type": "equity", "equity": equity})
    def log_risk(open_risk_pct: float, day_pnl_pct: float, rolling_dd_pct: float, mode: str = "normal"):
        return _write_event(EVENTS_DIR / "risk", {
            "ts": _now_ms(), "type": "risk",
            "open_risk_pct": open_risk_pct, "day_pnl_pct": day_pnl_pct,
            "rolling_dd_pct": rolling_dd_pct, "mode": mode,
        })
    def log_position_snapshot(exposures, avg_leverage):
        fn = SNAP_DIR / f"positions_{_now_ms()}_{random.randrange(1,1_000_000):06d}.json"
        with open(fn, "w", encoding="utf-8") as f:
            json.dump({
                "ts": _now_ms(), "exposures": exposures,
...
def get_bitget_candles(symbol: str, granularity_sec: int = 60, limit: int = 200) -> Optional[pd.DataFrame]:
    ...
    for url in BITGET_CANDLES_ENDPOINTS:
        try:
            ...
            df = pd.DataFrame(rows, columns=["ts","open","high","low","close"]).sort_values("ts")
...
def get_alpaca_bars(symbol: str, limit: int = 200) -> Optional[pd.DataFrame]:
    if not _ALPACA_AVAILABLE: return None
    try:
...
def signal_from_bars(df: pd.DataFrame, fast: int, slow: int) -> Optional[str]:
    if df is None or len(df) < max(fast, slow) + 2: return None
    closes = pd.Series(df["close"].values)
    f = sma(closes, fast); s = sma(closes, slow)
    if f.isna().any() or s.isna().any(): return None
    f_prev, f_curr = f.iloc[-2], f.iloc[-1]
    s_prev, s_curr = s.iloc[-2], s.iloc[-1]
    if f_prev <= s_prev and f_curr > s_curr: return "long"
    if f_prev >= s_prev and f_curr < s_curr: return "short"
    return None
...
# =============================
class Position:
    def __init__(self, symbol: str, venue: str, side: str, entry: float, atr_val: float, qty: float, rationale: str):
        self.symbol = symbol
        self.venue = venue
        ...
        log_trade_open(symbol=self.symbol, side=self.side, qty=self.qty, price=self.entry,
                       leverage=1.0, exchange=self.venue, strategy_id="sma_cross_atr", rationale=self.rationale)
    ...
    def exit_reason(self, px: float) -> Optional[Tuple[str,float]]:
        if self.side=="long":
            if px <= self.stop: return ("stop", self.stop)
            if px >= self.tp:   return ("tp", self.tp)
        else:
            if px >= self.stop: return ("stop", self.stop)
            if px <= self.tp:   return ("tp", self.tp)
        return None
...
class Portfolio:
    ...
    def key(self, venue, symbol): return f"{venue}:{symbol}"
    ...
    def open_position(self, venue, symbol, side, entry, atr_val, rationale):
        ...
        if venue=="alpaca": qty = max(1.0, round(qty))
    ...
    def maybe_exit(self, venue, symbol, px: float):
        ...
        log_trade_close(order_ref=f"{venue}-{symbol}-{int(time.time())}",
                        symbol=symbol, exit_price=float(exit_px),
                        profit=float(profit), pnl_pct=float(pnl_pct), fees=0.0)
    ...
    while True:
        ...
        try:
            ...
            for sym in CFG["symbols"]["bitget"]:
                ...
                if last_px is not None: prices[f"bitget:{sym}"] = last_px
                if last_px is not None: portfolio.maybe_exit("bitget", sym, last_px)
                if not pause_new_entries and df is not None:
                    sig = signal_from_bars(df, CFG["sma_fast"], CFG["sma_slow"])
                    a = last_atr(df, CFG["atr_len"])
                    if sig and a and last_px is not None and len(portfolio.positions)<CFG["max_concurrent_positions"]:
            ...
            for sym in CFG["symbols"]["alpaca"]:
                ...
                last_px = float(df["close"].iloc[-1]) if (df is not None and len(df)) else None
                if last_px is not None: prices[f"alpaca:{sym}"] = last_px
                if last_px is not None: portfolio.maybe_exit("alpaca", sym, last_px)
                if not pause_new_entries and df is not None:
                    sig = signal_from_bars(df, CFG["sma_fast"], CFG["sma_slow"])
                    a = last_atr(df, CFG["atr_len"])
                    if sig and a and last_px is not None and len(portfolio.positions)<CFG["max_concurrent_positions"]:
